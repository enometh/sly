...
;;
;; Version: 0.1
;; URL: https://github.com/mmgeorge/sly-mk-defsystem
...
;; support for working with mk-defsystem projects.  Ported from the original slime
;; contrib.  See <https://github.com/slime/slime/blob/master/contrib/slime-mk-defsystem.el>.
;;
;; See README.md
;;
;;; Code:
...
buffer's working directory"
  (interactive (list (sly-mk-defsystem-read-system-name)))
  (sly-mk-defsystem-oos system :compile))
...
(defun find-system (str)
  ...
  (let ((evaler (cond ((and (boundp 'slime-current-thread)
			    slime-current-thread)
		       'slime-eval)
		      ((and (boundp 'sly-current-thread)
			    sly-current-thread)
		       'sly-eval)

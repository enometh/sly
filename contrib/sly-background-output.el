...
either `sly-background-output-scroll-to-bottom' is T, or the point was
visible at the bottom of the screen when output began.

If  `sly-background-output-scroll-to-bottom' is TOP.
set window point to the beginning of the last output.
")
  ...
  (with-current-buffer (sly-background-output-buffer)
    (cl-flet ((doit
	       ...
	       (let ((sly-background-output-buffer-beg-pos (point))
		     (inhibit-read-only t)
		     (moving (= (point) sly-background-output-last-output-marker)))
		 (save-excursion
		   (goto-char sly-background-output-last-output-marker)
		   (insert string)
		   (set-marker sly-background-output-last-output-marker (point)))
		 (if moving (goto-char sly-background-output-last-output-marker))
	...
	(if (eq (selected-window) window)
	    (save-excursion (doit))
	  (when (eq sly-background-output-scroll-to-bottom 'top)
	  ...
	  (when (and (or (eq sly-background-output-scroll-to-bottom t)
			 restore-p)
		     window

...
(eval-when (:compile-toplevel :load-toplevel :execute)
  (slynk-require '(:slynk/arglists)))

;; madhu 191109. longest-compund-prefix (second return value of
;; completions) is not used at all - so always return NIL

(defpackage :slynk-c-p-c
  (:use :cl)
  (:import-from :slynk #:defslyfun #:guess-package
		#:*buffer-package* #:symbol-external-p
		#:tokenize-symbol #:package-names
		#:with-buffer-syntax
		;; slynk-arglists
		#:find-immediately-containing-arglist
		#:keyword-arg.keyword #:arglist.keyword-args #:arglist.any-args
		#:arglist-available-p
		#:parse-raw-form
		)
  (:import-from :slynk-completion  #:format-completion-set #:longest-common-prefix)
  (:import-from :slynk-backend  #:character-completion-set #:find-locally-nicknamed-package)
  (:export #:completions #:completions-for-keyword #:completions-for-character))

(in-package :slynk-c-p-c)

;; in sly-fancy-inspector - but we don't depend on sly-contribs
...
(defun longest-compound-prefix (completions &optional (delimiter #\-))
  ...
  (return-from longest-compound-prefix nil)
...
;;; From swank-c-p-c.lisp -- ILISP style Compound Prefix Completion
;;

...
(defslyfun create-mrepl (remote-id)
  (let* ((mrepl (make-instance
    ...
    (let ((target (maybe-redirect-global-io *emacs-connection*)))
      ...
      (when (typep *emacs-connection* 'slynk::multithreaded-connection)
	(setf (slynk::mconn.auto-flush-thread *emacs-connection*)
	      (slynk-backend::make-auto-flush-thread *standard-output*)))
